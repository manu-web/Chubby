...
std::string ClientLib::chubby_cell_handling_request_finder(){
    ...
    if(current_leader != "UNKNOWN"){
        ...
        Status status = chubby_map[current_leader]->Ping(&context, ping_request, &ping_response);
        if(status.ok())
    ...
    std::ranges::sample(chubby_cells.begin(), chubby_cells.end(), server_ports.begin(),connection_try_limit, gen);
    ...
    for (std::string &server_address : server_ports) {
        ...
        ClientContext context;
        Status status = chubby_map[server_address]->Ping(&context, ping_request, &ping_response);

        num_tries++;

        if (!status.ok()) {
            std::cerr << "Failed for server " << server_address << std::endl;
            if (num_tries == connection_try_limit) { //Set connection try limit to 3 or something 
...
int ClientLib::chubby_lock(std::string &path, std::string &locking_mode) {
  ...
  if(current_time < client_lease_timeout){
    ...
    ClientContext context;
    context.set_deadline(std::chrono::time_point<std::chrono::system_clock>(std::chrono::milliseconds(client_lease_timeout)));
    acquire_request.set_client_id(std::to_string(this->client_id)); //Have to take a lock here
    acquire_request.set_path(path);
    acquire_request.set_locking_mode(locking_mode);

    Status status = chubby_map[chubby_cell_handling_request]->AcquireLock(&context, acquire_request, &acquire_response);

    if(status.ok()){
        std::cout << "CHUBBY LOCK : Request went through, master is alive" << std::endl;
        ...
        if(current_time < client_lease_timeout + grace_period){
            ...
            ClientContext context;
            context.set_deadline(std::chrono::time_point<std::chrono::system_clock>(std::chrono::milliseconds(grace_period)));
    ...
    std::cerr << "CHUBBY LOCK : Can't acquire lock, client does not have a lease" << std::endl;
    return -2;
  }
...
int ClientLib::chubby_unlock(std::string &path) {
  ...
  if(current_time < client_lease_timeout){
    ...
    std::cerr << "CHUBBY UNLOCK : Can't release lock, client does not have a lease" << std::endl;
    return -2;
  }
